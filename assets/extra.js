        ...
        document.querySelectorAll(tag).forEach(el => {
            ...
            el.innerHTML = el.innerHTML.replace(/\[(p:)?([^\]]+)\]/gi, (match, prefix, content) => {
                ...
                if (prefix && prefix.toLowerCase() === "p:") {
                    const cleanName = displayContent.trim().toLowerCase();
                    const poke = Array.isArray(pokedex)
                        ? pokedex.find(p => p.name.toLowerCase() === cleanName)
                        : pokedex[cleanName];
                    ...
                    const statsHTML = `
                        <div class="stats-grid">
                            ${Object.entries(poke.base_stats)
                                                    .map(([stat, value]) => `
